...
def _ensure_cliente_panel_columns() -> None:
    ...
    try:
        with engine.begin() as conn:
            ...
            conn.execute(text("ALTER TABLE sedes ADD COLUMN IF NOT EXISTS whatsapp_telefono VARCHAR;"))
            conn.execute(text("ALTER TABLE sedes ADD COLUMN IF NOT EXISTS ultramsg_instance_id VARCHAR;"))
            conn.execute(text("ALTER TABLE sedes ADD COLUMN IF NOT EXISTS ultramsg_token VARCHAR;"))
